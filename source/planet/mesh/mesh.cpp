...
void rotate (Mesh* m, const Quaternion& q) {
...
const Tile* select_tile (const Planet* p, const Vector3& v) {
	...
	//navigate the mesh to find the closest tile
	bool closer = true;
	while(closer) {
		closer = false;
	...
	if (!point_in_tile(t, v)) {
		for (int i=0; i<t->edges; i++) {
			const Tile* nt = t->tile[i];
			if (point_in_tile(nt, v)) {
				t = nt;
				break;
			}
		}
